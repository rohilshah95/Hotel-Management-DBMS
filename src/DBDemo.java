...
//			ResultSet rs = null;
			try {
				...
				stmt.executeUpdate(
						"CREATE TABLE PROVIDES (HOTELID INT(4) ZEROFILL, NUMBER INT(2) ZEROFILL, STAFFID INT(3), SERVICEID INT, CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(NUMBER) REFERENCES ROOM(NUMBER) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(STAFFID) REFERENCES STAFF(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(SERVICEID) REFERENCES SERVICE(ID), PRIMARY KEY(HOTELID, NUMBER, STAFFID, SERVICEID), DATE DATE NOT NULL, TIME TIME NOT NULL)");
				stmt.executeUpdate(
