...
//			ResultSet rs = null;
			try {
				...
				stmt.executeUpdate(
						"CREATE TABLE BILL (ID INT PRIMARY KEY AUTO_INCREMENT, AMOUNT FLOAT NOT NULL, MODEOFPAYMENT VARCHAR(32) NULL, CARDNUMBER INT NULL, DISCOUNT FLOAT NOT NULL)");
				stmt.executeUpdate(
				...
				stmt.executeUpdate(
						"CREATE TABLE PROVIDES (HOTELID INT(4) ZEROFILL, NUMBER INT(2) ZEROFILL, STAFFID INT(3), SERVICEID INT, CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(NUMBER) REFERENCES ROOM(NUMBER) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(STAFFID) REFERENCES STAFF(ID), CONSTRAINT FOREIGN KEY(SERVICEID) REFERENCES SERVICE(ID), PRIMARY KEY(HOTELID, NUMBER, STAFFID, SERVICEID), TIMESTAMP DATETIME NOT NULL)");
				stmt.executeUpdate(
