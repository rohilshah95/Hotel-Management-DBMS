...
public class DBDemo {
	...
	public static void initializeDB() {
		try {
			...
			stmt.executeUpdate(
					"CREATE TABLE PROVIDES (HOTELID INT(4) ZEROFILL, NUMBER INT(2) ZEROFILL, STAFFID INT(3), SERVICEID INT, CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(NUMBER) REFERENCES ROOM(NUMBER) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(STAFFID) REFERENCES STAFF(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(SERVICEID) REFERENCES SERVICE(ID) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY(HOTELID, NUMBER, STAFFID, SERVICEID), DATE DATE NOT NULL, TIME TIME NOT NULL)");
			stmt.executeUpdate(
			...
			// stmt.executeUpdate("CREATE TRIGGER addpresidential AFTER INSERT ON ROOM
			// REFERENCING NEW ROW AS newRoom FOR EACH ROW
			// WHEN(newRoom.category='Presidential') INSERT INTO PRESIDENTIAL (number,
			// HotelID) VALUES (newRoom.number, newRoom.hotelId)");
			...
			stmt.executeUpdate(
					"INSERT INTO BILL (AMOUNT, MODEOFPAYMENT, CARDNUMBER, DISCOUNT) VALUES (0, 'credit', 1052, 0)");
			stmt.executeUpdate(
					"INSERT INTO BILL (AMOUNT, MODEOFPAYMENT, CARDNUMBER, DISCOUNT) VALUES (0, 'hotel credit', 3020, 5)");
			stmt.executeUpdate(
					"INSERT INTO BILL (AMOUNT, MODEOFPAYMENT, CARDNUMBER, DISCOUNT) VALUES (0, 'credit', 2497, 0)");
			stmt.executeUpdate(
					"INSERT INTO BILL (AMOUNT, MODEOFPAYMENT, CARDNUMBER, DISCOUNT) VALUES (0, 'cash', NULL, 0)");
