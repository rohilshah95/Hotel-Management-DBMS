...
public class DBDemo {
    ...
    stmt.executeUpdate("CREATE TABLE HOTEL (ID INT(4) zerofill PRIMARY KEY, NAME VARCHAR(32) NOT NULL, ADDRESS VARCHAR(64) NOT NULL, PHONE VARCHAR(32) NOT NULL, MANAGERID INT, CONSTRAINT FOREIGN KEY(MANAGERID) REFERENCES STAFF(ID) ON DELETE CASCADE ON UPDATE CASCADE)");
    stmt.executeUpdate("CREATE TABLE ROOM (NUMBER INT(2) zerofill, HOTELID INT(4), CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY(NUMBER, HOTELID), CATEGORY VARCHAR(32) NOT NULL, RATE FLOAT NOT NULL, AVAILABILITY TINYINT NOT NULL, MAXOCCUPANCY INT NOT NULL)");
    ...
    stmt.executeUpdate("CREATE TABLE HIRES (HOTELID INT, STAFFID INT, CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(STAFFID) REFERENCES STAFF(ID) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY(HOTELID, STAFFID))");
    stmt.executeUpdate("CREATE TABLE ASSIGNED (HOTELID INT, NUMBER INT, STAFFID INT, CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(NUMBER) REFERENCES ROOM(NUMBER) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(STAFFID) REFERENCES STAFF(ID) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY(HOTELID, NUMBER, STAFFID))");
    stmt.executeUpdate("CREATE TABLE PROVIDES (HOTELID INT, NUMBER INT, STAFFID INT, SERVICEID INT, CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(NUMBER) REFERENCES ROOM(NUMBER) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(STAFFID) REFERENCES STAFF(ID), CONSTRAINT FOREIGN KEY(SERVICEID) REFERENCES SERVICE(ID), PRIMARY KEY(HOTELID, NUMBER, STAFFID, SERVICEID), TIMESTAMP DATE NOT NULL)");
    stmt.executeUpdate("CREATE TABLE CHECKIN (CUSTOMERID INT, HOTELID INT, NUMBER INT, BILLID INT, CONSTRAINT FOREIGN KEY(CUSTOMERID) REFERENCES CUSTOMER(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(NUMBER) REFERENCES ROOM(NUMBER) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(BILLID) REFERENCES BILL(ID) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY(CUSTOMERID, HOTELID, NUMBER, BILLID), CHECKINTIME DATE NOT NULL, CHECKOUTTIME DATE, GUESTS INT NOT NULL)");
    stmt.executeUpdate("CREATE TABLE PRESIDENTIAL (NUMBER INT, HOTELID INT, CONSTRAINT FOREIGN KEY(NUMBER) REFERENCES ROOM(NUMBER) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FOREIGN KEY(HOTELID) REFERENCES HOTEL(ID) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY(NUMBER, HOTELID))");


		stmt.executeUpdate("INSERT INTO CUSTOMER VALUES (1001, 'David', '1980-01-30', '123', 'david@gmail.com', '12345678', 'abc, raleigh, nc', 1)");
		stmt.executeUpdate("INSERT INTO CUSTOMER VALUES (1002, 'Sarah', '1971-01-30', '465', 'sarah@gmail.com', '12445678', 'abcd, raleigh, nc', 1)");
    stmt.executeUpdate("INSERT INTO CUSTOMER VALUES (1003, 'Joseph', '1987-01-30', '9199673952', 'tkothari@ncsu.edu', '52345678', 'bc, raleigh, nc', 0)");
    stmt.executeUpdate("INSERT INTO CUSTOMER VALUES (1004, 'Lucy', '06-11-1994', '9199047775', 'rcoutin@ncsu.edu', '12468678', 'a, raleigh, nc', 0)");
